#!/usr/bin/perl -w
#
#   $Id: gen_argumentsmatchbuilder_N.pl 1254 2007-01-01 16:46:44Z ewald-arnold $
...
for($p = 1; $p <= $numArgs; ++$p) { print OUT "
    typedef typename I::OT" .$p . "    OT" . $p . ";  //!< internal shared member"; }

print OUT "

    typedef ConstraintSet" . $numArgs ."<" . $OTList. "> ConstraintSetType;  //!< internal shorthand
...
for($p = 2; $p <= $numArgs; ++$p) { print OUT "
                             , const ConstraintHolder<OT" .$p . "> &p" .$p. " = any() "; }
