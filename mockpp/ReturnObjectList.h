/** @file
  ...
  $Id: ReturnObjectList.h 1254 2007-01-01 16:46:44Z ewald-arnold $
...
class ReturnObjectList : public Verifiable
{
  public:
    ...
    typedef typename TypeTraits<T>::Type    ORT;
    typedef typename TypeTraits<T>::RefType  RRT;
  ...
  /** Add a next object to the end of the list.
    ...
    */
    ReturnObjectList& addObjectToReturn(const ORT &anObjectToReturn)
    {
  ...
  /** Adds a series of equal next objects to the end of the list.
    ...
    * @return reference to itself for chaining
    */
    ReturnObjectList& addObjectToReturn(const ORT &anObjectToReturn, unsigned count)
    {
  ...
  /** Sets a default object which is returned when the list is empty.
    ...
    */
    void setDefaultReturnValue(const ORT &defaultObj)
    {
      haveDefault = true;
  ...
  /** Returns the next object from the list. Each object it returned in the
    ...
    */
    RRT nextReturnObject()
    {
      String fmt = mockpp_i18n(MOCKPP_PCHAR("%1 has run out of objects."));
      fmt << getVerifiableName();

      if (hasMoreObjects() && haveDefault)
        return defaultObject;

      MOCKPP_ASSERT_TRUE_MESSAGE(fmt, hasMoreObjects());
		
		return myObjects[currentObject++];
  ...
  /** Transform the object's state to a human readable string.
    ...
    */
    String toString() const // virtual
    {
      ...
      for(int i = currentObject; i < myObjects.size(); ++i)
      {
  ...
  /** Returns true if there are more objects to be returned.
    ...
    */
    ...
    {
		return currentObject < myObjects.size();
    }
  ...
  /** Verify that there are no objects left within the list.
    ...
    */
    virtual void verify()
    {
      ...
      fmt << getVerifiableName() << size();

      MOCKPP_ASSERT_EQUALS_MESSAGE(fmt, (size_t)0, size());
    }

	 size_t size() const
	 {
      return myObjects.size() - currentObject;
	 }
  ...
  private:
    ...
    MOCKPP_STL::vector<ORT>  myObjects;
    bool                   haveDefault;
    ORT                      defaultObject;
};
